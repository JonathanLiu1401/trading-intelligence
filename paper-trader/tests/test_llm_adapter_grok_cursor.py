...
def test_is_grok_model_accepts_aliases():
    assert llm_adapter._is_grok_model("grok-4.5") is True
    assert llm_adapter._is_grok_model("xai/grok-4.5") is True
    assert llm_adapter._is_grok_model("cursor-grok-4.5-high") is True
    assert llm_adapter._is_grok_model("cursor-cli/cursor-grok-4.5-high") is True
...
def test_call_llm_routes_grok_to_strategy(monkeypatch):
    ...
    def fake_claude_call(prompt, timeout_s=90, model="grok-4.5"):
    ...
    out = llm_adapter.call_llm("grok-4.5", "hello", timeout=12)
    assert out == "ok"
    assert calls == [("hello", 12, "grok-4.5")]
