...
XAI_AUTH_PROFILE = os.environ.get(
    ...
    os.environ.get("PAPER_TRADER_XAI_AUTH_PROFILE", "xai:iamthemostproguy@gmail.com"),
)
...
def _xai_access_from_profiles(profiles: dict | None, *, log_prefix: str = "[claude_cli]") -> tuple[str | None, list[str]]:
...
def _refresh_openclaw_xai_oauth() -> bool:
    ...
    if now < _openclaw_xai_refresh_blocked_until:
        print("[claude_cli] OpenClaw xAI oauth probe skipped (cooldown)")
        return False
    cli = _openclaw_bin()
    if not cli:
        print("[claude_cli] OpenClaw CLI not found; cannot refresh xAI oauth")
    ...
    print("[claude_cli] asking OpenClaw to refresh xAI oauth store")
    try:
        proc = subprocess.run(
            cmd,
            capture_output=True,
        ...
        print(f"[claude_cli] OpenClaw xAI oauth probe failed: {type(e).__name__}")
    ...
    print(
        f"[claude_cli] OpenClaw xAI oauth probe rc={proc.returncode} "
...
def _load_xai_access_token() -> str | None:
    ...
    token, stale = _xai_access_from_profiles(profiles, log_prefix="[claude_cli]")
    if token:
        return token
    if stale:
        print(
            f"[claude_cli] xAI auth expired/missing for {len(stale)} profile(s); "
            "requesting OpenClaw refresh"
        )
    elif not raw:
        print("[claude_cli] xAI auth missing; requesting OpenClaw refresh")
    if _refresh_openclaw_xai_oauth():
        raw = _read_openclaw_auth_profiles()
        profiles = raw.get("profiles") if isinstance(raw, dict) else None
        token, _stale = _xai_access_from_profiles(profiles, log_prefix="[claude_cli]")
        if token:
            print("[claude_cli] xAI auth refreshed via OpenClaw")
            return token
    return None
