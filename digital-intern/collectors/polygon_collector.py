"""Polygon.io news collector — per-ticker, round-robin.

Free tier: 5 calls/minute. We poll with PER_TICKER_COOLDOWN_SEC to stay under.

Endpoint: GET https://api.polygon.io/v2/reference/news?ticker=NVDA&apiKey=KEY

Skips silently if POLYGON_API_KEY is unset.
"""
import hashlib
import json
import os
...
CURSOR_PATH = BASE_DIR / "data" / "polygon_cursor.json"
...
def _fetch_ticker(key: str, ticker: str) -> list:
    ...
    try:
        ...
        print(f"[polygon] {ticker} fetch error: {e}")
        return []
    if r.status_code == 429:
        print(f"[polygon] {ticker} rate-limited (429)")
        return []
    ...
    for it in data.get("results", []):
        ...
        publisher = (it.get("publisher") or {}).get("name", "unknown")
        out.append({
            "title": title,
            "link": link,
            "summary": summary,
            "published": published,
            "source": f"Polygon/{publisher}",
            "_ticker": ticker,
        })
    return out


def collect_polygon(batch: int = BATCH_PER_PASS) -> list:
    global _KEY_WARNED
    key = os.environ.get("POLYGON_API_KEY", "").strip()
    if not key:
        if not _KEY_WARNED:
            print("[polygon] POLYGON_API_KEY not set — skipping. "
        ...
        with ThreadPoolExecutor(max_workers=min(len(selected), 3)) as ex:
            ...
            for fut in as_completed(futures):
                try:
                    ...
                    print(f"[polygon] worker error: {e}")
...
if __name__ == "__main__":
    items = collect_polygon(batch=3)
    print(f"Got {len(items)} new Polygon articles")
    for a in items[:10]:
