...
RSS_INTERVAL        = _env_seconds("RSS_INTERVAL", 5)  # high-throughput default: re-poll RSS every 5s
WEB_INTERVAL        = _env_seconds("WEB_INTERVAL", 5)  # high-throughput default: scrape web as soon as prior pass cools down
REDDIT_INTERVAL     = _env_seconds("REDDIT_INTERVAL", 10)  # high-throughput default: re-poll Reddit every 10s
STOCKTWITS_INTERVAL = 15          # high-throughput mode: re-poll StockTwits every 15s
TICKER_INTERVAL     = _env_seconds("TICKER_INTERVAL", 10)  # high-throughput default: re-fetch ticker news every 10s
SCORE_INTERVAL      = _env_seconds("SCORE_INTERVAL", 5)  # high-throughput default: run scoring pass every 5s
ALERT_CHECK         = 10          # high-throughput mode: check urgent alerts every 10s
PURGE_INTERVAL      = 6 * 3600   # purge old data every 6h
GDELT_INTERVAL      = _env_seconds("GDELT_INTERVAL", 120)  # high-throughput default: full GDELT sweep every 2min
...
YAHOO_TICKER_RSS_INTERVAL = _env_seconds("YAHOO_TICKER_RSS_INTERVAL", 60)  # high-throughput default: Yahoo per-ticker RSS every 1min
...
MARKET_MOVERS_INTERVAL  = _env_seconds("MARKET_MOVERS_INTERVAL", 60)  # high-throughput default: Yahoo gainers/losers/most-active every 1min
YAHOO_TRENDING_INTERVAL = _env_seconds("YAHOO_TRENDING_INTERVAL", 60)  # high-throughput default: Yahoo trending tickers every 1min
...
BENZINGA_INTERVAL       = _env_seconds("BENZINGA_INTERVAL", 60)  # Benzinga analyst-ratings RSS sweep every 1min
...
FINVIZ_INTERVAL        = _env_seconds("FINVIZ_INTERVAL", 60)  # Finviz per-ticker news table round-robin every 1min
SEEKINGALPHA_INTERVAL  = _env_seconds("SEEKINGALPHA_INTERVAL", 60)  # Seeking Alpha breaking-news RSS every 1min
...
GLOBENEWSWIRE_INTERVAL  = _env_seconds("GLOBENEWSWIRE_INTERVAL", 120)  # GlobeNewswire financial press releases every 2min
PRNEWSWIRE_INTERVAL     = _env_seconds("PRNEWSWIRE_INTERVAL", 120)  # PR Newswire press releases every 2min
...
FINANCIAL_BLOGS_INTERVAL = _env_seconds("FINANCIAL_BLOGS_INTERVAL", 120)  # InvestorPlace, Motley Fool, Nasdaq RSS every 2min
INVESTMENT_RESEARCH_BLOGS_INTERVAL = _env_seconds("INVESTMENT_RESEARCH_BLOGS_INTERVAL", 180)  # investment research blogs every 3min
...
WORKER_POLL_INTERVAL_SECS = {
    ...
    "energy_news": ENERGY_NEWS_INTERVAL,
    "eia": EIA_INTERVAL,
...
def _notify_state_transition(name: str, old: str, new: str, count: int) -> None:
    ...
    if new == "degraded":
        ...
        return
    try:
        from notifier.discord_notifier import send as _discord_send

        _discord_send(msg, is_alert=True)
    except Exception as e:
...
def ticker_worker(store: ArticleStore):
    ...
    from collectors.ticker_news import collect_ticker_news as _collect_ticker_news

    bo = Backoff("ticker", base=5.0, cap=300.0)
    while _running:
        try:
            articles = _collect_ticker_news()
...
def sec_enforcement_worker(store: ArticleStore):
    log.info("[sec_enforcement_worker] started")
    bo = Backoff("sec_enforcement", base=10.0, cap=600.0)
    while _running:
        try:
            articles = collect_sec_enforcement()
            _ingest(store, articles, "sec_enforcement")
            try:
                source_health.record_result("sec_enforcement", len(articles))
            except Exception as he:
                log.warning(f"[sec_enforcement_worker] source_health error: {he}")
            _worker_last_ok["sec_enforcement"] = time.time()
            bo.reset()
        except Exception as e:
            log.warning(f"[sec_enforcement_worker] error: {e}; backing off {bo.peek():.0f}s")
            bo.sleep(lambda: _running)
            continue
        _sleep(SEC_ENFORCEMENT_INTERVAL)
...
def extended_hours_movers_worker(store: ArticleStore):
    ...
    from collectors.extended_hours_movers import collect as _collect_extended_hours_movers
    ...
    while _running:
        try:
            articles = _collect_extended_hours_movers()
...
def nasdaq_dividends_worker(store: ArticleStore):
    log.info("[nasdaq_dividends_worker] started")
    bo = Backoff("nasdaq_dividends", base=60.0, cap=1800.0)
    while _running:
        try:
            articles = collect_nasdaq_dividends()
            _ingest(store, articles, "nasdaq_dividends")
            try:
                source_health.record_result("nasdaq_dividends", len(articles))
            except Exception as he:
                log.warning(f"[nasdaq_dividends_worker] source_health error: {he}")
            _worker_last_ok["nasdaq_dividends"] = time.time()
            log.debug(f"[nasdaq_dividends] cycle ok ({len(articles)} new)")
            bo.reset()
        except Exception as e:
            log.warning(f"[nasdaq_dividends_worker] error: {e}; backing off {bo.peek():.0f}s")
            bo.sleep(lambda: _running)
            continue
        _sleep(NASDAQ_DIVIDEND_INTERVAL)
...
def putcall_ratio_worker(store: ArticleStore):
    ...
    from collectors.putcall_ratio_collector import collect_putcall_ratio as _collect_putcall_ratio

    bo = Backoff("putcall_ratio", base=60.0, cap=1800.0)
    while _running:
        try:
            articles = _collect_putcall_ratio()
...
def earnings_surprise_worker(store: ArticleStore):
    ...
    from collectors.earnings_surprise_collector import (
        collect_earnings_surprises as _collect_earnings_surprises,
    )

    bo = Backoff("earnings_surprise", base=60.0, cap=900.0)
    while _running:
        try:
            articles = _collect_earnings_surprises()
...
def gdpnow_worker(store: ArticleStore):
    log.info("[gdpnow_worker] started")
    bo = Backoff("gdpnow", base=120.0, cap=3600.0)
    while _running:
        try:
            articles = collect_gdpnow()
            _ingest(store, articles, "gdpnow")
            try:
                source_health.record_result("gdpnow", len(articles))
            except Exception as he:
                log.warning(f"[gdpnow_worker] source_health error: {he}")
            _worker_last_ok["gdpnow"] = time.time()
            log.debug(f"[gdpnow] cycle ok ({len(articles)} new)")
            bo.reset()
        except Exception as e:
            log.warning(f"[gdpnow_worker] error: {e}; backing off {bo.peek():.0f}s")
            bo.sleep(lambda: _running)
            continue
        _sleep(GDPNOW_INTERVAL)
...
def vix_ts_worker(store: ArticleStore):
    ...
    from collectors.vix_term_structure import collect as _collect_vix_ts

    bo = Backoff("vix_ts", base=30.0, cap=600.0)
    while _running:
        try:
            # collector inserts directly; count emitted rows
            articles = _collect_vix_ts()
            n = len(articles)
            try:
...
def dxy_worker(store: ArticleStore):
    ...
    from collectors.dxy_collector import collect as _collect_dxy

    bo = Backoff("dxy", base=30.0, cap=600.0)
    while _running:
        try:
            articles = _collect_dxy()
            n = len(articles)
            try:
...
def commodity_futures_worker(store: ArticleStore):
    ...
    from collectors.commodity_futures_collector import (
        collect as _collect_commodity_futures,
    )

    bo = Backoff("commodity_futures", base=30.0, cap=600.0)
    while _running:
        try:
            articles = _collect_commodity_futures()
            n = len(articles)
            try:
...
def energy_news_worker(store: ArticleStore):
    log.info("[energy_news_worker] started")
    bo = Backoff("energy_news", base=60.0, cap=900.0)
    while _running:
        try:
            articles = collect_energy_news()
            _ingest(store, articles, "energy_news")
            try:
                source_health.record_result("energy_news", len(articles))
            except Exception as he:
                log.warning(f"[energy_news_worker] source_health error: {he}")
            _worker_last_ok["energy_news"] = time.time()
            log.debug(f"[energy_news] cycle ok ({len(articles)} new rows)")
            bo.reset()
        except Exception as e:
            log.warning(f"[energy_news_worker] error: {e}; backing off {bo.peek():.0f}s")
            bo.sleep(lambda: _running)
            continue
...
def rsi_extremes_worker(store: ArticleStore):
    ...
    from collectors.rsi_extremes_collector import collect_rsi_extremes as _collect_rsi_extremes

    bo = Backoff("rsi_extremes", base=30.0, cap=600.0)
    while _running:
        try:
            articles = _collect_rsi_extremes()
...
def etf_fund_flows_worker(store: ArticleStore):
    ...
    from collectors.etf_fund_flows import collect_etf_flows as _collect_etf_flows

    bo = Backoff("etf_fund_flows", base=120.0, cap=3600.0)
    while _running:
        try:
            articles = _collect_etf_flows()
...
# ── Worker: Investment research blogs (CMT, Lyn Alden, Felder, AWoCS…) ───────
def investment_research_blogs_worker(store: ArticleStore):
    log.info("[investment_research_blogs_worker] started")
    bo = Backoff("investment_research_blogs", base=60.0, cap=900.0)
    while _running:
        try:
            articles = collect_investment_research_blogs()
            _ingest(store, articles, "investment_research_blogs")
            try:
                source_health.record_result("investment_research_blogs", len(articles))
            except Exception as he:
                log.warning(f"[investment_research_blogs_worker] source_health error: {he}")
            _worker_last_ok["investment_research_blogs"] = time.time()
            log.debug(f"[investment_research_blogs] cycle ok ({len(articles)} new rows)")
            bo.reset()
        except Exception as e:
            log.warning(f"[investment_research_blogs_worker] error: {e}; backing off {bo.peek():.0f}s")
            bo.sleep(lambda: _running)
            continue
...
def earnings_transcript_worker(store: ArticleStore):
    ...
    bo = Backoff("earnings_transcript", base=60.0, cap=600.0)
    while _running:
        try:
            articles = collect_earnings_transcripts()
            _ingest(store, articles, "earnings_transcript")
            try:
                source_health.record_result("earnings_transcript", len(articles))
            except Exception as he:
                log.warning(f"[earnings_transcript_worker] source_health error: {he}")
            _worker_last_ok["earnings_transcript"] = time.time()
            log.debug(f"[earnings_transcript] cycle ok ({len(articles)} new rows)")
            bo.reset()
        except Exception as e:
            log.warning(f"[earnings_transcript_worker] error: {e}; backing off {bo.peek():.0f}s")
            bo.sleep(lambda: _running)
            continue
        _sleep(EARNINGS_TRANSCRIPT_INTERVAL)
...
# ── Worker: S&P 500 market valuation (CAPE, P/E, earnings yield) — every 4h ──
def market_valuation_worker(store: ArticleStore):
    log.info("[market_valuation_worker] started")
    bo = Backoff("market_valuation", base=300.0, cap=3600.0)
    while _running:
        try:
            articles = collect_market_valuation()
            _ingest(store, articles, "market_valuation")
            try:
                source_health.record_result("market_valuation", len(articles))
            except Exception as he:
                log.warning(f"[market_valuation_worker] source_health error: {he}")
            _worker_last_ok["market_valuation"] = time.time()
            log.debug(f"[market_valuation] cycle ok ({len(articles)} new rows)")
            bo.reset()
        except Exception as e:
            log.warning(f"[market_valuation_worker] error: {e}; backing off {bo.peek():.0f}s")
            bo.sleep(lambda: _running)
            continue
        _sleep(MARKET_VALUATION_INTERVAL)


# ── Worker: Portfolio P/L snapshot — every 5min ─────────────────────────────
def portfolio_pl_worker(store: ArticleStore):
    log.info("[portfolio_pl_worker] started")
    from collectors.portfolio_pnl import write_pl_snapshot as _write_pl_snapshot

    bo = Backoff("portfolio_pl", base=5.0, cap=300.0)
    while _running:
        try:
            snap = _write_pl_snapshot()
            if snap is not None:
                s = snap.get("summary", {})
...
def sentiment_trends_worker(store: ArticleStore):
    ...
    from ml.sentiment_trends import write_trends as _write_score_trends

    while _running:
        try:
            with _store_lock:
                data = _write_score_trends(store)
...
def scorer_worker(store: ArticleStore):
    ...
    from watchers.urgency_scorer import (
        BATCH_SIZE as _urgency_batch_size,
    ...
    while _running:
        try:
            ...
            if llm_candidates:
                ...
                for j in range(0, len(llm_candidates), _urgency_batch_size):
                    llm_urgent += _score_batch(
                        llm_candidates[j:j + _urgency_batch_size], store
                    )
            with _store_lock:
...
def _price_alert_universe() -> list[str]:
    ...
    return sorted(set(PORTFOLIO_TICKERS) | set(_live_portfolio_tickers))
...
def price_alert_worker(store: ArticleStore):
    ...
    from collectors.stock_data import _fetch_one, get_stock_data
    from notifier.discord_notifier import send as _discord_send
...
def continuous_trainer_worker(store: ArticleStore):
    ...
    from ml.trainer import train_continuous as _train_continuous
    ...
    while _running:
        try:
            metrics = _train_continuous(store, heartbeat=_heartbeat)
...
def recursive_labeler_worker(store: ArticleStore):
    ...
    from ml.recursive_labeler import run_recursive_labeling as _run_recursive_labeling
    ...
    while _running:
        try:
            ...
            summary = _run_recursive_labeling(store)
...
def heartbeat_worker(store: ArticleStore):
    ...
    from analysis.claude_analyst import analyze as _analyze
    from collectors.earnings_calendar import get_earnings
    from collectors.options_monitor import format_options_block, get_options_data
    from collectors.portfolio_pnl import format_pnl_block, get_portfolio_pnl
    ...
    while _running:
        ...
        if now - last >= HEARTBEAT_INTERVAL:
            try:
                ...
                briefing = _analyze(top, stocks, earnings)
